...
public partial class DororongPresenter
{
    ...
    internal bool UpdateHunting(PetSnapshot snapshot, DirectInteractionSnapshot direct,
        PointerSample pointer, TimeSpan elapsed, bool pressed)
    {
        ...
        _huntSession.Advance(_huntDelta,_huntNear,blocked);
        if (blocked) _huntGaze.Reset();
        return _huntSession.IsActive;
    }

    private void ApplyHunting(PetSnapshot snapshot, DirectInteractionSnapshot direct, bool closed)
    {
        _huntingImage = null;
        if (!_huntSession.IsActive || _sittingRequested || _locomotionBlocked || _edgePerch.IsAttached ||
        ...
        var gaze = _huntGaze.Advance(_huntDelta,target,flip,_huntNear);
        _huntDelta = 0; // An immediate extra render must not advance gaze twice.
        _huntFrame=(int)Math.Clamp(Math.Round(_huntSession.FrameAgeSeconds*60),0,156);
