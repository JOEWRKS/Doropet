...
For every existing source/native sample array, set the normal endpoints to the first and last legacy sample centers, encoded as coordinate times eight. Preserve the legacy arrays as provenance during this task; no coverage test may use them after the new fields exist. Preserve each existing source `Fill`/`Ink` as `SourceFill`/`SourceInk`. For every native hair anchor, select `NativeFill` as the brightest and `NativeInk` as the darkest **opaque** existing native sample by Rec. 709 luminance, with ordinal `Y,X` ties. For every body normal, preserve its existing source `Fill` as `SourceFill` and select `NativeFill` as the brightest **opaque** existing native sample by the same tie rule. The opaque-only selection set is required because every chosen reference coordinate must be opaque and transparent RGB has no visible optical luminance. Write all selected coordinates literally into the PSD1; the authoring script validates them but never rewrites the fixture.
