...
The final mask identity, component ownership, contour fingerprint, endpoint pair, continuous authority, and geometric support were reviewed and frozen before the completed feasibility work. The source-only gate swept one global distance radius from `0.25` through `4.00` source pixels in `1/64`-pixel increments using the then-active `8x8` area-coverage rule and recorded a definitive empty fifteen-normal 10% intersection. That invocation, its normals, and its intervals remain diagnostic evidence only. It is not an automated release requirement, does not select or forbid a runtime candidate, and is not repeated. Task 5A keeps `W = 2.20898670201159` for one-sided `E` support and `W/2 = 1.104493351005795` for centered `C` support, then applies the fixed F optical transfer inside those supports. The native `0.35` and `0.50` measurements likewise remain diagnostics rather than release gates.
...
Canonical source identity, immutable predecessor-seed identity, final body-ownership mask identity, 82-component/167-pixel predecessor expansion evidence, exact component-membership and canonical-contour fingerprints, writable-alpha validity, protected-component/normal exclusion, exact `(118,151)` / `(161,116)` endpoint authority, real-generator/committed hash equality, native-96 dimensions and alpha-zero RGB hygiene, delivered-source final-mask confinement, source-alpha preservation, delivered protected-pixel equality, continuous source-derived all-pixel smooth fill with no seed-coordinate retain exception, fixed `8x8` separate E/C coverage, frozen `E -> W` / `C -> W/2` support, exact `2.5`/`0.125` F coverage transfer, deterministic six-component/twelve-pixel resize-only proxy identity with exact canonical membership SHA-256 `2B9CB6D649884DA2DC826963E3258A23DAFAAE9A1B071335B168834746463A54`, identical open/closed body reconstruction and proxy input, explicit scaled-eye confinement, forbidden eye-patch absence, closed-lid and mouth preservation, 96-DPI presenter arrangement, native-coordinate alpha hit testing, and presenter state-to-frame mapping are automated contracts.

The exact-art test independently integrates optical stroke coverage along the literal post-ownership continuous normals for every exposed front, foot, underside, valley, center, rear-leg, and rear-rim segment. Those body normals are fixed against the reviewed final contour and are not emitted or imported by the generator; the clean-hair anchors and targets remain unchanged. The 225px body-normal measurements, completed 10% intersection, native `0.35` hair-match result, and native `0.50` spread result are reported diagnostics, not release assertions; candidate F's exact visual selection supersedes them as gates. A count-only, connected-component-only, integer-pixel-sum-only, or candidate-derived identity test is insufficient. Tests independently encode resize-proxy membership and require SHA-256 `2B9CB6D649884DA2DC826963E3258A23DAFAAE9A1B071335B168834746463A54`; they also prove that changing the immutable seed, ownership, endpoint, contour geometry or kind, subpixel factor, either support radius, either F coverage multiplier, all-pixel smooth-fill rule, proxy connectivity/threshold/count/membership, source alpha, delivered protected RGB, resize count, or open/closed body/proxy equality causes its named contract to fail. Reversing a raw segment or permuting raw segment input must produce the same canonical hash and pass. Exact source/native F hashes and behavioral generator/direct-pipeline equality are mandatory; no diagnostic measurement may silently substitute a PASS for a hash or visual failure.
