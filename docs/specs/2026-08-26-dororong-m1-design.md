...
Body-outline normalization occurs only after that resize. At native 96px the generator clears the downsampled irregular ink inside one hand-owned narrow edit band to locally derived opaque body fill, then rasterizes one continuous 1.35px optical contour with round joins and caps through deterministic 4x coverage sampling. The contour covers the exposed front leg, undersides and valleys, center leg, rear leg, and rear rim. The correction preserves the uncorrected 96px alpha at every coordinate, changes RGB only within the body band, and is byte-identical between open and closed frames wherever applied. Hair, head, face, mouth, eyes, rose, bow, ribbons, no-tail geometry, and layer order receive no local redraw; their only permitted production difference from the 225px authority is the deterministic whole-frame resize and, for the closed frame, the reviewed eye derivation.
...
Canonical source identity, real-generator/committed hash equality, native-96 dimensions and alpha-zero RGB hygiene, body-band RGB confinement, alpha preservation, identical open/closed body correction, fixed hair/body optical profiles, connected contour support, explicit scaled-eye confinement, forbidden eye-patch absence, closed-lid and mouth preservation, 96-DPI presenter arrangement, native-coordinate alpha hit testing, and presenter state-to-frame mapping are automated contracts. Those checks do not prove cross-process click-through, focus preservation, or user acceptance in the produced executable; those remain actual-Windows observations.
