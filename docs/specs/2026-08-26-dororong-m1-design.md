...
Within that mask, the generator removes the source's variable-width body ink through a deterministic source-derived fill field and reconstructs the outline from the approved body boundary. Source alpha is authoritative: reconstruction may change RGB inside the reviewed body mask, including RGB carried by partially transparent body-edge pixels, but it may not change alpha at any coordinate. The body boundary and legal occlusion endpoints remain those of the pinned source; the new stroke is clipped to the body mask and grows only inward. Protected source pixels outside the mask remain byte-identical before the final whole-frame resize and are composited over the reconstructed body in their original layer order.

The body fill field comes only from reviewed local opaque body-fill samples in the pinned source. Before a candidate is rendered, the samples are fixed per exposed anatomical segment; each erased source-ink pixel takes the nearest eligible sample in source coordinates, with ordinal `Y,X` tie-breaking. This preserves the white-body appearance without sampling the transparent background or a protected character part. The outline RGB is the component-wise median of the dark samples at the fixed clean hair-outline anchors. The reconstructed outline uses that one color rule and one global width; it may use fractional coverage for antialiasing, but it may not use segment-specific widths, hand-tuned 96px pixels, or local corrective strokes. Hair, head, face, mouth, eyes, rose, bow, ribbons, no-tail geometry, part connectivity, and layer order receive no reconstruction.

Before rendering a candidate, fixed clean hair-outline anchors establish the target optical weight. At least six anchors must cover straight, diagonal, and curved hair strokes without junctions or occlusions. Their alpha-weighted dark-ink coverage normal to the line is measured at source scale and after the deterministic 96px resize; the median is the body target. Coverage is the sum across the fixed normal scan of source alpha multiplied by normalized luminance darkness between the preselected local fill and outline-color samples, clamped to `[0,1]`, so it represents an equivalent opaque-pixel width. The generator's one width value is calibrated once against those preselected anchors, recorded with the mask hash and anchors in the task evidence, and then frozen before candidate review. Candidate pixels cannot define their own target or permitted variation.
...
Canonical source identity, body-mask identity, real-generator/committed hash equality, native-96 dimensions and alpha-zero RGB hygiene, exact body-mask confinement, source-alpha preservation, source-boundary and occlusion-endpoint preservation, protected-pixel equality, source-derived fill provenance, a single global body-outline width/color rule, fixed clean-hair optical references, identical open/closed body reconstruction, explicit scaled-eye confinement, forbidden eye-patch absence, closed-lid and mouth preservation, 96-DPI presenter arrangement, native-coordinate alpha hit testing, and presenter state-to-frame mapping are automated contracts.

The exact-art test measures alpha-weighted optical stroke coverage along literal, independently selected normals for every exposed front, foot, underside, valley, center, rear-leg, and rear-rim segment. Those normals and the clean-hair anchors are fixed from the source before a candidate is reviewed and are not emitted by the generator. At 225px, every body sample must be within 10% of the frozen source-scale hair median. At native 96px, every body sample must be within `0.35` equivalent opaque pixel of the frozen native hair median, and the difference between the heaviest and lightest body samples must not exceed `0.50` equivalent opaque pixel. A count-only, connected-component-only, or candidate-derived test is insufficient. The test also proves that a width mutation, mask-boundary mutation, alpha mutation, protected-part mutation, or different open/closed body reconstruction fails.
