...
- Controller-approved Task 5A visual refinement: 2026-08-28
...
The user selected the replacement direction: preserve one global source-scale width, but compute both raster coverage and optical measurement in continuous subpixel space. The approved mechanism uses the binary mask and source-derived visible contour as geometric authority, evaluates an `8x8` deterministic subpixel grid inside each affected 225px pixel, and measures fixed hair/body normals by continuous line integration. At that stage it could not fall back to the rejected pixel-center seeds, alter the then-active optical checks, or introduce segment-specific widths, local corrective strokes, post-resize edits, or candidate-derived reference targets. The later 2026-08-28 Task 5A decision changes the optical coverage transfer and status of those checks without changing this geometry.
...
For the Task 4B recovery, the frozen source hair median `2.20898670201159` defined one intended full rendered stroke thickness with the unchanged component-wise median outline-color rule. An exposed `E` segment used inward radius `W`; an internal legal continuation `C` used centered radius `W/2 = 1.104493351005795`. This established the reviewed geometric support without segment-specific geometry or local correction. Task 5A retains those exact radii as support but supersedes Task 4B's direct raw-coverage blend with the calibrated optical transfer below.

Task 4B changed only the distance-side interpretation. It left the exact body ownership mask; canonical `E`/`C` contour geometry; legal endpoints `(118,151)` and `(161,116)`; then-active source-derived fill field; fixed `8x8` sampling; source-alpha, alpha-zero, and protected-pixel rules; pinned source identity; single whole-frame 225-to-96 resize; and open/closed body equality unchanged. Task 5A later replaces the fill output and optical transfer under explicit rules while keeping those authority, geometry, alpha, silhouette, protected-art, and resize boundaries. Segment-specific support widths, local corrective strokes, coordinate patches, post-resize body edits, candidate-derived targets, and hand-authored replacement curves remain forbidden.

The fifteen source/body normals and their recorded empty intervals remain diagnostic evidence only. Their 10% source intersection is removed from automated release requirements and must not be used to forbid the approved recovery or trigger a second feasibility sweep. The later Task 5A inspection also proved that the native `0.35` hair-match and `0.50` body-spread measurements contradict the exact visually selected candidate; they remain reported diagnostics, not release gates. Exact identity/invariant checks, artifact inspection, and actual Windows user acceptance remain required.

The reversible attempt-2 candidate remains historical evidence only. It used `W = 2.20898670201159`, median outline RGB `(26,2,10)`, source-225 SHA-256 `8302307105F76A99C531AA8FD61908B58FF1C15537B703F6B6EFC20E895DCBE3`, and native-96 SHA-256 `4329C62523C9E9BC0D3223037506E06160DB32B31F5876B7EBC68C95BE4F560B`. Its earlier visual PASS and candidate binding are withdrawn: exact main-controller inspection of the native-96 image and nearest-neighbor 4x views on white and dark backgrounds found a pale outer body line, an over-dark continuation, and a gray body speckle. Those hashes must not be used as active production acceptance targets.

### Decision record — controller-approved Task 5A optical refinement (2026-08-28)

A controlled D/E/F calibration changed one variable set at a time while retaining the same source, ownership mask, canonical contour and kinds, legal endpoints, source alpha, silhouette, and protected character art. Candidate F was selected. Its geometric support remains exactly `W = 2.20898670201159` for exposed `E` and `W/2 = 1.104493351005795` for continuation `C`, with outline RGB `(26,2,10)`. It does not expand, move, or redraw that support. Instead, let `Ecoverage` and `Ccoverage` be the separate raw `8x8` area fractions inside those frozen supports; the final optical coverage is `max(clamp(Ecoverage * 2.5, 0, 1), clamp(Ccoverage * 0.125, 0, 1))`.

Candidate F remains pinned as source-225 SHA-256 `AB5E0F0990980F0393CF02FFDCDC15329EE7BC8F770D70D3DD68FCB775AD5A5A`. Deterministic production generation and the direct pipeline now reproduce that source identity and the integrated repository runtime assets exactly: open/native-96 SHA-256 `F4C9B2CCE253522345F12D29F6CC634ACD0DE1C3151D7C923460E3D5EBA73B9A` and closed/native-96 SHA-256 `9B5411E88C031CA7D6542F6A1B3FD8E8C080BC061D96D4246B82011B9BF5A0BA`. The main controller's repository, native-96, and nearest-neighbor 4x inspection on white and exact RGB `(18,20,28)` backgrounds passed those exact integrated hashes, including the previously failing outer body, continuation, and gray-speckle surfaces. The calibration provenance remains bound jointly to evidence root `.superpowers/sdd/2026-08-27-dororong-complete-body-ownership-outline/task-5a-visual-calibration/second-set`, manifest SHA-256 `95FC6C493C371563D0BA24C97B443A456275C2745C7C3F4121FD3A296D03BE36`, and the exact F identities above; it is inadmissible if any one differs. Full ExactArt and the final automated integration suite passed. Invocation evidence is the ignored report `.superpowers/sdd/2026-08-27-dororong-complete-body-ownership-outline/task-5-runtime-integration/report.md`, SHA-256 `446B5BEEFDAAAD70FA7DB8AB77D98D5B8F371564989844F7AAD9EE0EA83E2061`. Exact live WPF rendering and user acceptance remain `UNVERIFIED`; no implementation may silently promote a different evidence set, manifest, candidate hash, failed visual surface, automated result, or repository inspection to those live outcomes.

This Task 5A record is historical and is superseded by the actual-Windows correction below. Its repository-only visual PASS must not be used as an active acceptance baseline.

### Decision record — actual-Windows E-only thin-outline correction (2026-08-28)

Actual-Windows attempt 7 rejected the integrated F artifact: the outer body line still read materially heavier than the unchanged head/hair line, and both legal internal continuation strokes remained visible after merely reducing their optical opacity. The user's screenshot is preserved at `docs/verification/evidence/attempt-7-user-open-body-fail.png`; attempt 7 is `FAIL`, and all later live checks in that attempt remain `UNVERIFIED`. The generated image-model exploration was also rejected because it redrew the head, face, hair, rose, bow, ribbons, proportions, and occlusions. It is not a production asset or visual authority.

The active correction keeps the pinned canonical source, complete ownership mask, smooth fill, canonical `382 E / 2 C` geometry, endpoints, source alpha, protected art, resize proxy, and single 225-to-96 resize unchanged. It changes the one exposed support radius to `W = 1.5` source pixels, keeps exposed optical gain `2.5`, and sets continuation optical gain to exactly `0.0`. The two `C` records remain structural endpoint/occlusion authority only and contribute no rendered ink. Active raster coverage is therefore exactly `clamp(Ecoverage * 2.5, 0, 1)`; `Ccoverage`, its distance field, and any mutation of that field must be byte-inert in delivered source and runtime output.

Three controlled E-only candidates used `W = 1.5`, `1.625`, and `1.75` with every other input frozen. Main-controller native-96 and nearest-neighbor 4x inspection on white and RGB `(18,20,28)` selected `W = 1.5`: its native high-opacity body median is `0.75` pixel against the unchanged hair median `0.6875`, while the other two candidates measure `0.875`. All three removed the internal continuation ink, but only `1.5` is the active production decision. Calibration evidence is bound to `.superpowers/sdd/2026-08-28-dororong-e-only-thin-outline/candidate-calibration`, manifest SHA-256 `0E62125BA02C787AB758110B538892AA0BD06F50409EC684F94A207422ACDFEF`, and selected native-96 SHA-256 `238AC7F0ACC765ABC40AE3E13543E088BC3F694C0D4FBC99BDFD99648D94B511`.

The direct pipeline and generator reproduce source-open SHA-256 `D1F0770CBCA95FC79B5E68642D78A5A48077834495C9ECDBCD73B34545AC94FF`, source-closed SHA-256 `70BC4C304CA8DF2D38A2FC2A451CCA8BF7A3099B8B4C81551869891511406551`, runtime-open SHA-256 `238AC7F0ACC765ABC40AE3E13543E088BC3F694C0D4FBC99BDFD99648D94B511`, and runtime-closed SHA-256 `F48AB174F6DEE6C92F04E7363F854CC92AA1ED53504A728F7F69E8F1D0A0167E`. A causal regression first failed on F with body median `1.125` and a first continuation-field difference at `(159,111)`, then passed with body median `0.75` and byte-inert continuation geometry. Automated and repository visual evidence did not promote the then-pending current-Windows observation; the exact E-only body received its separate live PASS only in attempt 8, as recorded in section 13.
...
Within the final ownership mask, the generator replaces the source RGB with a deterministic source-derived smooth fill field before applying any new outline coverage. Every body-owned pixel is assigned that field, including coordinates that supplied eligible source samples; every newly claimed old outline and antialias-support pixel is reconstructed, and no body-owned legacy ink remains outside reconstruction. Source alpha is authoritative and never changes. The delivered source-225 evidence changes RGB only inside the reviewed final mask: every mask-zero source pixel remains byte-identical there, and protected character art is composited in its original layer order. The new stroke is clipped to the frozen body ownership and grows only within the active `E -> 1.5` support; `C` geometry contributes no visible coverage.
...
Contour identity is canonical rather than implementation-order-dependent. Multiply every segment coordinate by two to encode exact half-pixel integer units; orient each segment so its first endpoint precedes its second by ordinal `(Y2,X2)`; remove exact duplicates; encode records as `kind|startY2|startX2|endY2|endX2` with kind `E` for exposed edge and `C` for continuation; sort those records ordinally by the five fields; join them with LF and hash the UTF-8 bytes. Independent tests reimplement ownership and contour derivation, use the same literal endpoint pair without importing production constants, and require exact selected-component membership and canonical contour hashes. For every final-mask-255 pixel centered at integer source coordinate `(x,y)`, the generator evaluates all 64 fixed sample points `(x-0.5+(i+0.5)/8, y-0.5+(j+0.5)/8)` for `i,j` in `0..7`; no separate outline-band predicate may omit a writable pixel. Each fixed sample independently evaluates its actual minimum Euclidean distance to canonical `E` and to canonical `C`. Active `Ecoverage` is the fraction whose nearest-`E` distance is at most `1.5`; the final raster coverage is exactly `clamp(Ecoverage * 2.5, 0, 1)`. Canonical `C` distances remain available for authority and regression checks but have optical multiplier `0.0` and must not change any output byte. The transfer changes optical opacity only: samples outside the frozen `E -> 1.5` support remain uncovered, and no segment, endpoint, or silhouette coordinate moves. The resulting coverage blends one smooth fill RGB with outline RGB `(26,2,10)` while preserving source alpha; coverage is never inferred from candidate pixels.

The body fill field comes only from the pinned source and is fixed before optical coverage is applied. An eligible fill seed must have processed-source alpha `255`, cleaned predecessor-seed value `255`, ordinary unweighted geometric distance greater than `8.0` source pixels from the approved `E union C` contour, every RGB channel at least `225`, and `max(R,G,B)-min(R,G,B) <= 8`; newly claimed components can never supply samples. Each eligible seed contributes its exact source RGB as an interpolation sample, but it has no retain-source output exception. Every final-mask pixel, including eligible-seed coordinates and regardless of contour distance, is assigned a deterministic inverse-distance interpolation of the nearest eight eligible samples, using weight `1/(1+d^2)`, ordinal `Y,X` tie-breaking, and nearest-integer ties-to-even channel rounding. Generation throws before writing evidence if fewer than eight eligible samples exist, if a selected sample is invalid, or if any final-mask pixel cannot be assigned eight samples. It also proves that every source pixel inside the final mask with any RGB channel below `225` is reconstructed. Thus newly owned, dark legacy, and eligible-sample coordinates all use one smooth field with no preserve-original output branch. The outline RGB remains the component-wise median of production-owned clean hair-outline samples. Hair, head, face, mouth, eyes, rose, bow, ribbons, no-tail geometry, part connectivity, and layer order receive no delivered source-225 reconstruction.

Immediately before the one shared 225-to-96 resize—and only on an unsaved resize-input clone—the generator applies the same smooth field to every enclosed four-connected mask-zero neutral component that does not touch the frame, has at most `7` pixels, and has `max(R,G,B)-min(R,G,B) <= 8`. Component discovery and eligibility are topology/color based and coordinate-independent. Canonical resize-proxy membership sorts the pixels within each selected component by ordinal `(Y,X)`, encodes each record as `C|count|Y,X;...`, sorts records by their first `(Y,X)`, joins them with LF and no trailing LF, and hashes the UTF-8 bytes. The pinned input must yield exactly `6` proxy components totaling `12` pixels, resize-proxy membership SHA-256 `2B9CB6D649884DA2DC826963E3258A23DAFAAE9A1B071335B168834746463A54`, and the diagnosed `7`-pixel gray island with bounds `(144,159)-(147,160)`. The delivered source-225 evidence remains byte-identical outside the final mask; only the transient clone receives this resize proxy. The proxy never changes alpha, is not saved, is not a post-resize or per-coordinate correction, and is applied identically to the open and closed resize inputs.
...
The final mask identity, component ownership, contour fingerprint, endpoint pair, and continuous authority were reviewed and frozen before the completed feasibility work. The source-only gate swept one global distance radius from `0.25` through `4.00` source pixels in `1/64`-pixel increments using the then-active `8x8` area-coverage rule and recorded a definitive empty fifteen-normal 10% intersection. That invocation, its normals, and its intervals remain diagnostic evidence only. It is not an automated release requirement, does not select or forbid a runtime candidate, and is not repeated. Task 5A's `W = 2.20898670201159` plus rendered `C` support is historical failed evidence. The active actual-Windows correction uses one-sided `E` support `W = 1.5`, fixed E gain `2.5`, and zero rendered `C` gain. The native `0.35` and source fifteen-normal measurements likewise remain diagnostics rather than release gates; the active causal regression requires native body high-opacity median no greater than `0.75` and continuation-distance byte-inertness.
...
Canonical source identity, immutable predecessor-seed identity, final body-ownership mask identity, 82-component/167-pixel predecessor expansion evidence, exact component-membership and canonical-contour fingerprints, writable-alpha validity, protected-component/normal exclusion, exact `(118,151)` / `(161,116)` endpoint authority, real-generator/committed hash equality, native-96 dimensions and alpha-zero RGB hygiene, delivered-source final-mask confinement, source-alpha preservation, delivered protected-pixel equality, continuous source-derived all-pixel smooth fill with no seed-coordinate retain exception, fixed `8x8` sampling, active `E -> 1.5` support, exact `2.5` exposed transfer, zero continuation contribution with continuation-distance byte-inertness, deterministic six-component/twelve-pixel resize-only proxy identity with exact canonical membership SHA-256 `2B9CB6D649884DA2DC826963E3258A23DAFAAE9A1B071335B168834746463A54`, identical open/closed body reconstruction and proxy input, explicit scaled-eye confinement, forbidden eye-patch absence, closed-lid and mouth preservation, 96-DPI presenter arrangement, native-coordinate alpha hit testing, and presenter state-to-frame mapping are automated contracts.

The exact-art test independently integrates optical stroke coverage along the literal post-ownership continuous normals for every exposed front, foot, underside, valley, center, rear-leg, and rear-rim segment. Those body normals are fixed against the reviewed final contour and are not emitted or imported by the generator; the clean-hair anchors and targets remain unchanged. The historical 225px body-normal intersection and native `0.35`/`0.50` measurements remain reported diagnostics. The active regression instead executes the real generator, requires native high-opacity body median `<= 0.75`, and proves that replacing the complete continuation-distance field cannot change any delivered source byte. A separate `C gain 0 -> 1` mutation proves that this continuation-inertness assertion is causally sensitive rather than vacuous. A count-only, connected-component-only, integer-pixel-sum-only, or candidate-derived identity test is insufficient. Tests independently encode resize-proxy membership and require SHA-256 `2B9CB6D649884DA2DC826963E3258A23DAFAAE9A1B071335B168834746463A54`; they also prove that changing the immutable seed, ownership, endpoint, contour geometry or kind, subpixel factor, `E` support, E gain, zero C gain, all-pixel smooth-fill rule, proxy connectivity/threshold/count/membership, source alpha, delivered protected RGB, resize count, or open/closed body/proxy equality causes its named contract to fail. Reversing a raw segment or permuting raw segment input must produce the same canonical hash and pass. Exact active source/native hashes and behavioral generator/direct-pipeline equality are mandatory; no diagnostic measurement may silently substitute a PASS for a hash or visual failure.

A candidate must be inspected source-first at the 225px production scale, at native 96px, and at nearest-neighbor enlargement on white and RGB `(18,20,28)` backgrounds. The comparison covers the full frame plus focused views of each exposed outer, foot, underside, valley, rear-rim, legal occlusion segment, and every proxy-component location. Any new or newly exposed branch or endpoint, displaced silhouette, opaque light fringe, missing span, broken leg/valley contour, pale outer body line, over-dark continuation, gray body speckle, protected-part damage, or body segment visibly heavier/lighter than the unchanged hair reference is a failure. Open and closed-eye frames receive separate native checks, and their body plus resize-proxy inputs must be identical outside the eye regions.
...
## 13. Phase-1 body-art closure and next visual phase (2026-08-28)

Actual-Windows attempt 8 ran the exact E-only published artifact whose active outline is `W=1.5`, E gain `2.5`, and C gain `0`. The user supplied a live screenshot and said `좋다`. This is direct acceptance of the phase-1 body-outline correction: the canonical no-tail, three-leg/two-valley silhouette remains intact, the exposed body line reads as one thin weight matching the head/hair reference, no stroke protrudes, and the former internal chest/shoulder continuation is absent rather than merely faded. The evidence is `docs/verification/evidence/attempt-8-user-phase-1-acceptance-and-closed-eyes-next.png`, SHA-256 `5DC4871E60F4A5AC7EFC0D9A33392AF19C825A7A51AEC349746E73E4268DCCE1`.

This closes the body-outline correction only. It does not override the section-12 milestone definition of done. In the same observation, the user explicitly directed the next visual phase to revise the way Dororong closes its eyes and to add character-preserving expressions and motion for mouse and situation-specific actions. The screenshot proves that the current closed eyes are on the face rather than detached; it does not prove that the expression or animation is accepted. Closed-eye expression/motion quality and the remaining actual-Windows interaction/non-interference items therefore remain `UNVERIFIED`, and broader Milestone 1 remains `PARTIAL`.

The next visual phase must preserve the exact canonical character identity, approved body outline, no-tail interpretation, protected head/face/hair/rose/bow/ribbon art, alpha hit-testing, transparent click-through, and existing behavior priority/timing contracts. Before implementation it defines the smallest state-to-expression/motion set for IDLE, WALK, CURIOUS, STARTLED, CLICK_REACTION, DRAGGED, and SLEEP. It does not automatically add progression, sound, accounts, cloud features, settings, auto-start, or polished multi-monitor support.
